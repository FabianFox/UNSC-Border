...
unsc_voting <- tibble(
  url = paste0("https://digitallibrary.un.org/search?ln=en&rg=100&jrec=", 
               seq(1, 2480, 100), 
...
unsc_voting <- unsc_voting %>%
  mutate(id = pluck(voting_df, "value"),
         id = pluck(id, 3))
           
# Clean the listed table of information on the resolutions
unsc_voting <- unsc_voting %>%
  mutate(voting_df = map(voting_df, 
...
unsc_voting <- unsc_voting %>%
  mutate(voting_df = map(voting_df, 
                         ~ mutate(., country = str_split(vote, "(\\sY\\s|\\sN\\s|\\sA\\s)"),
...
# Remove resolutions w/o vote
unsc_voting <- unsc_voting %>%
  filter(res_voted_on == TRUE) 

# Error: All nested columns must have the same number of elements.
# not due to different number of member states and votes
# Might run on each data set and then back combine...
unsc_voting <- unsc_voting %>%
  mutate(voting_df = map(voting_df, 
...
unsc_voting <- unsc_voting %>%
  mutate(voting_df = map(voting_df, 
                         ~ mutate(., country = str_trim(str_remove_all(country, "([:lower:]|[:punct:])"), "left"),
